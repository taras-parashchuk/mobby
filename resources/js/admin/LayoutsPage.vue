<template>
    <div class="table-list-container">
        ...
        <div class="singleForm singleForm--inModulesTpl" v-if="layouts.length">
            <div class="moduleTemplates">
                <menu class="menu menu--inModulesTpl">
                    <ul class="menu__list">
                        <li class="menu__item" v-for="(layout, index) in layouts">
                            <div class="menu__heading" :class="{'menu__heading--active': openLayout === index }"
                ...
                <main class="mainContent mainContent--noHeight"
                      v-bar="{useScrollbarPseudo: true, preventParentScroll: false}">
                    <div>
                        <div class="grid grid--3 grid--gap_25">
                            <div class="layout" v-for="position in positions" :key="openLayout.index + '-' + position">
                                ...
                                <div class="layout__content">
                                    ...
                                    <draggable
                                        ...
                                        <div class="layoutModule"
                                             v-for="(module, moduleIndex) in layouts[openLayout].modules"
                                             :key="layouts[openLayout].name + ' ' + position + ' ' + moduleIndex"
                                             v-if="module.position === position">
                                            <div class="flex flex--align-center">
                                                <icon class="icon icon-delete mr--8" icon="cancel"
                                                      @click.native="layouts[openLayout].modules.splice(moduleIndex, 1)"></icon>
                                                {{module.name}}
                                            </div>
                                    ...
                                    <v-select
                                            ...
                                            :searchable="true"
                                            :options="modules"
                                            @input="(module) => addModuleToSelectedList(module, position)"
        ...
        <widget-actions :store="isChangedLayout ? 'update' : false"></widget-actions>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            update() {
                ...
                axios.put('/admin/layouts/' + this.layouts[this.openLayout].id, this.layouts[this.openLayout]).then(httpResponse => {
