...
<body>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{ route('unisharp.lfm.upload') }}" role='form' id='uploadForm' name='uploadForm' method='post' enctype='multipart/form-data' class="dropzone">
            ...
            @php
              $itemId = str_replace(request()->get('items_type').'-', '', base64_decode(request()->get('item')));
              $if_isset_desc_path = request()->input('in_description', false) ? 'description' : '';
            @endphp

            <input type='hidden' name='working_dir' id='working_dir' value="images/{{request()->get('items_type')}}/{{ $itemId ?: 'tmp' }}{{$if_isset_desc_path ? '/'.$if_isset_desc_path : ''}}">
  ...
  <script src="{{asset('js/libraries/filemanager.index.js') }}"></script>
